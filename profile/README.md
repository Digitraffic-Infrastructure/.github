...
|--------------------------------------------|---------------------------------------------------------------------------------|:------------------------:|:------------------------:|
| **[EtsiLibrary](../ETSILibrary.md)**      | A library for generating, encoding, and decoding V2X messages compliant with the ETSI standard. |      `4.0.3` <!-- ETSILIB_VERSION --> | `1.0.35` <!-- CETSILIB_VERSION --> |
| **[Connect](../Connect.md)**            | Tools and utilities for establishing connections using BI and SI interfaces.           |      `5.0.1` <!-- CONNECT_VERSION --> | `1.0.21` <!-- CCONNECT_VERSION --> |
| **[PKCS11SecurityModule](../PKCS11.md)**            | Cryptographic interface for interacting with HSMs for message encryption, decryption, signing and signature verification.            |      `3.0.15` <!-- PKCS11_VERSION --> | `Unavailable` |
...
```sh
dotnet add package Digitraffic.Connect --version 1.0.21
```
```sh
...
```xml
<ItemGroup>
  <PackageReference Include="Digitraffic.Connect" Version="1.0.21" />
</ItemGroup>
```
```xml
<ItemGroup>
